...
include $_SERVER['DOCUMENT_ROOT'] . '/templates/shopNavMenu.php';
if (isset($_GET['new']) || $_SERVER['REQUEST_URI'] == '/new') {
    $isNew = 1;
} else {
    $isNew = 0;
}

if (isset($_GET['sale']) || $_SERVER['REQUEST_URI'] == '/sale') {
    $isSale = 1;
} else {
    $isSale = 0;
}
...
<main class="shop-page">
    ...
    <section class="shop__filter filter">
      <div class="filter__wrapper">
        ...
        <ul class="filter__list">
          <li>
            <a class="filter__list-item <?= activeHighlight('/')?>" href="/" id="0">Все</a>
          </li>
          <li>
            <a class="filter__list-item <?= activeHighlight('/women')?>" href="/women" id="1">Женщины</a>
          </li>
          <li>
            <a class="filter__list-item <?= activeHighlight('/man')?>" href="/man" id="2">Мужчины</a>
          </li>
          <li>
            <a class="filter__list-item <?= activeHighlight('/kids')?>" href="/kids" id="3">Дети</a>
          </li>
          <li>
            <a class="filter__list-item <?= activeHighlight('/accessories')?>" href="/accessories" id="4">Аксессуары</a>
          </li>
        </ul>
      </div>
        <form id="filter-form" >
        <div class="filter__wrapper">
          ...
          <div class="filter__range range">
            ...
            <div class="range__res">
              <span class="range__res-item min-price"><?= isset($_GET['min'])?dataFilter($_GET['min']):'350'?> руб.</span>
                <input type="hidden" class="min-price field" name="min"
                    <?= !empty($_GET['min'])?'value=' . dataFilter($_GET['min']):'value=350'?>>
              <span class="range__res-item max-price"><?= isset($_GET['max'])?dataFilter($_GET['max']):'32000'?> руб.</span>
                <input type="hidden" class="max-price field" name="max"
                    <?= !empty($_GET['max'])?'value=' . dataFilter($_GET['max']):'value=32000'?>>
            </div>
          </div>
        </div>

        <fieldset class="custom-form__group">
          <input type="checkbox" name="new" id="new" class="custom-form__checkbox" <?= $isNew?'checked':''?>>
          <label for="new" class="custom-form__checkbox-label custom-form__info" style="display: block;">Новинка</label>
          <input type="checkbox" name="sale" id="sale" class="custom-form__checkbox" <?= $isSale?'checked':''?>>
          <label for="sale" class="custom-form__checkbox-label custom-form__info" style="display: block;">Распродажа</label>
        </fieldset>
            <button class="button" type="submit" id="custom-form__button" style="width: 100%">Применить</button>

    </section>

    <div class="shop__wrapper">
          <?php
          include $_SERVER['DOCUMENT_ROOT'] . '/include/productList.php';
          ?>
    </div>
  </section>
  <section class="shop-page__order" hidden="">
    <div class="shop-page__wrapper">
      ...
      <form action="index.php" method="post" class="custom-form js-order">
        <fieldset class="custom-form__group">
        ...
        <fieldset class="custom-form__group shop-page__comment">
          ...
          <fieldset class="custom-form__group shop-page__comment">
              <legend class="custom-form__title custom-form__title--comment">Общая сумма заказа</legend>
              <p class="custom-form__price"></p>
              <p class="custom-form__raw-price" hidden></p>
